...
class Users extends Controller with MongoController {
  ...
  def deleteUser(firstName: String, lastName: String, age: Int) = Action.async(parse.json) {
    request =>
      request.body.validate[User].map {
        user =>
          // find our user by first name and last name
          val nameSelector = Json.obj("firstName" -> firstName, "lastName" -> lastName, "age" -> age)
